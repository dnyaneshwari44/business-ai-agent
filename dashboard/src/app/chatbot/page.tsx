...
export default function ChatbotPage() {
  ...
  const [employees, setEmployees] = useState<
    { login: string; avatar_url?: string; assigned_issues: number }[]
  >([]);
  ...
  const applyConversationState = useCallback(
    (nextConversations: ChatConversation[], preferredId?: string | null) => {
      ...
      setActiveId((current) => {
        ...
        return desiredId &&
          nextConversations.some(
            (conversation) => conversation.id === desiredId,
          )
          ? desiredId
    ...
    async (
      ...
      const conversationsToSync = sourceConversations.filter(
        (conversation) =>
          ...
          !pendingDeletes.includes(conversation.id),
      );
  ...
  useEffect(() => {
    if (conversations.length > 0) {
      ...
      if (!result.success) {
        setStorageWarning(
          "Local storage limit reached. Chat history could not be saved.",
        );
      } else if (result.prunedCount > 0) {
        setStorageWarning(
          `Local storage limit reached. Oldest ${result.prunedCount} chat(s) were pruned to save space.`,
        );
        const prunedList = conversations.slice(
          0,
          conversations.length - result.prunedCount,
        );
  ...
  const confirmEscalate = useCallback(
    async (msgIndex: number, assigneeName?: string) => {
      ...
      try {
        ...
        if (!res.ok) {
          ...
          showToast(
            `Escalation failed: ${err.error || "Unknown error"}`,
            "error",
          );
        } else {
  ...
  const sendMessage = useCallback(async () => {
    ...
    const conversationTitle =
      messages.length === 0 ? deriveTitle(userMsg) : activeConv.title;
    ...
    let streamParseErrorShown = false; // To avoid spamming errors if the stream is malformed
    setInput("");
    setCompletedNodes([]);
        ...
        for (const part of parts) {
          for (const line of part.split("\n")) {
            ...
            try {
                  ...
                  if (nodeName && nodeName !== "__start__") {
                    setCompletedNodes((prev) => {
                      ...
                      return [
                        ...
                        {
                          name: nodeName,
                          friendlyName: friendlyNodeName(nodeName),
                        },
                      ];
                    });
                  }
              ...
              /* Robustness: if an SSE chunk fails to parse, show a warning but keep the stream alive for the next chunks */
        ...
        updateActiveMessages((prev) => {
          ...
          if (last?.role === "assistant") {
            updated[updated.length - 1] = {
              ...
              content:
                last.content ||
                `Error: Could not reach the AI agent. ${errMsg}`,
            };
          ...
          <div className="chat-header">
            ...
            >
              <h2 className="chat-header-title">
                <ChatbotIcon size={20} color="var(--accent-blue)" /> AI Business
                Agent
              </h2>
            ...
            <p className="chat-header-subtitle">
              Ask questions about your business data, financials, employees, and
              more.
                ...
                >
                  {msg.role === "assistant" && (
                    <div className="chat-avatar assistant-avatar">
                      <ChatbotIcon size={16} color="white" />
                    </div>
                  )}
                  <div
                  ...
                  >
                    ...
                    {msg.intent && (
                      <span className="chat-intent-badge">{msg.intent}</span>
                    )}
                  </div>
                  {msg.role === "user" && (
                  ...
                  <div className="agent-flow-pipeline">
                    {completedNodes.map((node, idx) => {
                      ...
                      return (
                        <div
                          key={node.name}
                          className="agent-flow-step-wrapper"
                        >
                          {idx > 0 && <div className="agent-flow-connector" />}
                          <div
                            className={`agent-flow-step ${isActive ? "active" : "done"}`}
                          >
                            {isActive ? (
